...
int SymQueryIndex(SymTable ip, char* name)
{
  char **Names         = (char**) ip->FieldVals[ip->NameField];
   int initial = hash_string(name, ip->NumIndices);
   int i       = initial;
   int count   = 1;
...
static void OverflowVectors(SymTable ip)
{
   int   i, j, NewSlots, NewSize, OldSize ;
  Generic Val;
   Generic *p;
...
void SymDumpEntryByIndex(SymTable ip, int index)
{
	...
	fprintf(stderr, "Single Entry of Symbol Table (%lx)\n", (unsigned long)ip);
		...
		if (ip->FieldVals[ip->NameField][index] != 0) {
			...
			for(i=0;i<ip->NumFields;i++) {
				if (i != ip->NameField && ip->FieldVals[i] != 0)
					fprintf(stderr, "  %-8s %-12s  %-30s  %-10ld  0x%-8lx\n",
						"", "", ip->FieldNames[i], (long)ip->FieldVals[i][index],
						(unsigned long)ip->FieldVals[i][index]);
			}
		}
	}
}
...
{
	int i,j;
	fprintf(stderr, "Dump of Symbol Table (%lx)\n", (unsigned long)ip);
	fprintf(stderr, "%-10s %-12s  %-30s  %-10s  %-10s\n",
		"Index", "Name", "Field", "Dec Value", "Hex Value");
	...
	for(j=FIRST_SLOT;j<ip->NumSlots;j++)
	{
		if (ip->FieldVals[ip->NameField][j] != 0) {
			...
			for(i=0;i<ip->NumFields;i++) {
				if (i != ip->NameField && ip->FieldVals[i] != 0)
					fprintf(stderr, "  %-8s %-12s  %-30s  %-10ld  0x%-8lx\n",
						"", "", ip->FieldNames[i], (long)ip->FieldVals[i][j],
						(unsigned long)ip->FieldVals[i][j]);
			}
		}
	}
}
